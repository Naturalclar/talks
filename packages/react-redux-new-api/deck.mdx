...
```jsx 1:19 title="今までの react-redux"
...
```jsx 13:15 subtitle="mapDispatchToProps"
import React from "react";
import { connect } from "react-redux";
import { someAction } from "../actions";
...
- 複数の値を Object として取ってくる場合：
  ...
  - `equalityFn` に `shallowequal` を入れる
  - `reselect` を使う

<Notes>
  第２引数の equalyFn には react-redux が提供している shallowequal を
</Notes>
